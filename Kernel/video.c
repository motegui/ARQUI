...
void putPixel(uint32_t hexColor, uint32_t x, uint32_t y)
{
    uint8_t *screen = (uint8_t *)((uint64_t)(VBE_mode_info->framebuffer));
    uint32_t offset = VBE_mode_info->pitch * y + x * 3;
    for (int m = 0; m < INITIAL_FONT_SIZE; m++)
    {
        for (int n = 0; n < INITIAL_FONT_SIZE; n++)
        {
            screen[offset + VBE_mode_info->pitch * n + 3 * m] = hexColor & 0xFF;
            screen[offset + VBE_mode_info->pitch * n + 3 * m + 1] = (hexColor >> 8) & 0xFF;
            screen[offset + VBE_mode_info->pitch * n + 3 * m + 2] = (hexColor >> 16) & 0xFF;
        }
    }
}
...
void copyPixel(uint32_t new_x, uint32_t new_y, uint32_t old_x, uint32_t old_y)
{
    ...
    for (int m = 0; m < INITIAL_FONT_SIZE; m++)
    {
        for (int n = 0; n < INITIAL_FONT_SIZE; n++)
        {
            uint32_t offset_old = VBE_mode_info->pitch * (old_y * INITIAL_FONT_SIZE + n) + old_x * INITIAL_FONT_SIZE * 3 + 3 * m;
            uint32_t offset_new = VBE_mode_info->pitch * (new_y * INITIAL_FONT_SIZE + n) + new_x * INITIAL_FONT_SIZE * 3 + 3 * m;
            screen[offset_new] = screen[offset_old];
            screen[offset_new + 1] = screen[offset_old + 1];
            screen[offset_new + 2] = screen[offset_old + 2];
        }
...
void putLetter(int character, uint32_t x, uint32_t y, int color)
{
    unsigned char *bitMap = charMap(character);
    for (int j = 0; j < CHAR_HEIGHT; j++)
    {
        for (int i = 0; i < CHAR_WIDTH; i++)
        {
            ...
            if (bit == 1)
            {
                // Draw a INITIAL_FONT_SIZEd rectangle instead of a single pixel
                for (int m = 0; m < INITIAL_FONT_SIZE; m++)
                {
                    for (int n = 0; n < INITIAL_FONT_SIZE; n++)
                    {
                        putPixel(color, x + (CHAR_WIDTH - 1 - i) * INITIAL_FONT_SIZE + m, y + j * INITIAL_FONT_SIZE + n);
                    }
                }
            }
        }
...
void putArray(char *array, uint32_t x, uint32_t y, int color)
{
    ...
    while (array[i] != '\0')
    {
        if (aux_x + CHAR_HEIGHT * INITIAL_FONT_SIZE > VBE_mode_info->width)
        {
            aux_x = 0;
            aux_y = aux_y + CHAR_HEIGHT * INITIAL_FONT_SIZE;
        }
        putLetter(array[i], aux_x, aux_y, color);
        i++;
        aux_x = aux_x + CHAR_WIDTH * INITIAL_FONT_SIZE;
    }
}
...
void moveUpScreen()
{
    for (int y = 0; y < VBE_mode_info->height - CHAR_HEIGHT * INITIAL_FONT_SIZE; y++)
    {
        for (int x = 0; x < VBE_mode_info->width; x++)
        {
            copyPixel(x, y, x, y + CHAR_HEIGHT * INITIAL_FONT_SIZE);
        }
    }

    for (int y = VBE_mode_info->height - CHAR_HEIGHT * INITIAL_FONT_SIZE; y < VBE_mode_info->height; y++)
    {
        for (int x = 0; x < VBE_mode_info->width; x++)
        {
    ...
    pointer_y = pointer_y - CHAR_HEIGHT * INITIAL_FONT_SIZE;
    pointer_x = 3;
}
...
void putBackspace()
{
    if (pointer_x == 0 && pointer_y == 0)
        return;
    pointer_x = pointer_x - CHAR_WIDTH * INITIAL_FONT_SIZE;
    for (int j = 0; j < CHAR_HEIGHT * INITIAL_FONT_SIZE; j++)
    {
        for (int i = 0; i < CHAR_WIDTH * INITIAL_FONT_SIZE; i++)
        {
            putPixel(0x000000, pointer_x + (CHAR_WIDTH - 1) * INITIAL_FONT_SIZE - i, j + pointer_y);
        }
    }
    if (pointer_x < 0 && pointer_y > 0)
    {
        pointer_x = VBE_mode_info->width - CHAR_WIDTH * INITIAL_FONT_SIZE;
        pointer_y = (pointer_y >= CHAR_HEIGHT * INITIAL_FONT_SIZE) ? pointer_y - CHAR_HEIGHT * INITIAL_FONT_SIZE : 0;
    }
}
...
void putLetterNext(int character, int color)
{
    if (character == '\b')
    {
        putBackspace();
    ...
    if (pointer_x + CHAR_WIDTH * INITIAL_FONT_SIZE > VBE_mode_info->width)
    {
        pointer_x = 3;
        pointer_y = pointer_y + CHAR_HEIGHT * INITIAL_FONT_SIZE;
    }
    if (pointer_y + CHAR_HEIGHT * INITIAL_FONT_SIZE > VBE_mode_info->height)
    {
        moveUpScreen();
    }
    putLetter(character, pointer_x, pointer_y, color);
    pointer_x = pointer_x + CHAR_WIDTH * INITIAL_FONT_SIZE;
}
